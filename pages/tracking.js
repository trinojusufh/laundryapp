      ...
      <section className="relative min-h-screen overflow-hidden bg-gradient-to-b from-white via-green-50 to-white px-6 pt-28 pb-20">
        ...
        "
          ...
          <div className="w-full">
            ...
            {/* INPUT */}
            <div className="mt-8 flex flex-col sm:flex-row gap-3 bg-white/70 backdrop-blur-xl border shadow-lg rounded-2xl p-3">
              <input
                value={invoice}
                onChange={(e) => setInvoice(e.target.value)}
                onKeyDown={handleKeyDown}
                placeholder="Masukkan Invoice ID (INV-001)"
                className="flex-1 px-4 py-3 bg-transparent outline-none rounded-xl border border-gray-200 sm:border-0"
              />

              <button
                onClick={handleTrack}
                className="w-full sm:w-auto bg-green-500 text-white px-6 py-3 rounded-xl hover:scale-105 active:scale-95 transition font-medium"
              >
                Track
              </button>
            </div>
                ...
                <div className="mt-6 space-y-3">
                  {result.timeline.map((t, i) => (
                    <div key={i} className="flex items-center gap-3">
                      ...
                      />
                      <p
                        className={t.done ? "text-green-600" : "text-gray-400"}
                      >
                        {t.step}
                      </p>
                    </div>
